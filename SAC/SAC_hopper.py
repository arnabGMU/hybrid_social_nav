...
from SAC.utils.utils_hopper import soft_update, hard_update, soft_update_obs_train, soft_update_obs_train_
from SAC.model_hopper import GaussianPolicy, QNetwork, DeterministicPolicy
from encoder.obs_encoder_hopper import ObsEncoder
...
class SAC(object):
    def __init__(self, num_inputs, action_space, args):
        ...
        if args.obs_train:
            ...
            # self.critic_optim = Adam([{'params': self.critic.parameters()},\
            #     {'params': self.obs_encoder.parameters(), 'lr': 0.00001}], lr=args.lr)
            pass   
        else:
            self.critic_optim = Adam(self.critic.parameters(), lr=args.lr)
        # Critic target
        ...
        if self.policy_type == "Gaussian":
            ...
            if args.obs_train:
                ...
                # self.policy_optim = Adam([{'params': self.policy.parameters()},\
                #     {'params': self.obs_encoder.parameters(), 'lr': 0.00001}], lr=args.lr)
                #self.optimizer = Adam(list(self.critic.parameters()) + list(self.policy.parameters()) + list(self.obs_encoder.parameters()), lr=args.lr)
            else:
                self.policy_optim = Adam(self.policy.parameters(), lr=args.lr)
    ...
    def update_parameters(self, memory, batch_size, updates):
        ...
        if self.args.obs_train:
            ...
            next_task_obs = next_task_obs.to(self.device).float()
            next_waypoints_obs = next_waypoints_obs.to(self.device).float()
            next_local_map = next_local_map.to(self.device).float()
            next_prev_action = next_prev_action.to(self.device).float()
            next_pedestrian_map = next_pedestrian_map.to(self.device).float()
            next_ped_pos_obs = next_ped_pos_obs.to(self.device).float()

            state_batch = self.obs_encoder(task_obs, waypoints_obs, local_map, prev_action, pedestrian_map, ped_pos_obs)
            #state_batch_clone = self.obs_encoder(task_obs, waypoints_obs, local_map, prev_action, pedestrian_map, ped_pos_obs)
            with torch.no_grad():
        ...
        if self.args.obs_train:
            ...
            # torch.nn.utils.clip_grad_norm_(list(self.obs_encoder.parameters()),
            #                              0.5)
            # torch.nn.utils.clip_grad_norm_(list(self.obs_encoder.parameters()) + list(self.critic.parameters()),
            #                              0.5)
            # torch.nn.utils.clip_grad_norm_(self.critic.parameters(),
            #                              0.5)
            # print("critic opt", list(self.critic.parameters()))
            # print("obs", list(self.obs_encoder.parameters()))
        ...
        #loss = qf_loss * 0.5 + policy_loss * 0.1

        if self.args.obs_train == True:
            self.policy_optim.zero_grad()
            policy_loss.backward()
            # print("policy opt", list(self.policy.parameters()))

            # torch.nn.utils.clip_grad_norm_(list(self.obs_encoder.parameters()) + list(self.policy.parameters()),
            #                              0.5)
            ...
            # loss.backward()
            # torch.nn.utils.clip_grad_norm_(list(self.obs_encoder.parameters()) + list(self.critic.parameters()) + list(self.policy.parameters()),
            #                              0.5)
            # self.optimizer.step()
        else:
            self.policy_optim.zero_grad()
